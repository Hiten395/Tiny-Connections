...
public class GroundManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            GameObject gridline = Instantiate(gridDot, new Vector3(i - nodelimits.width / 4, 0, 0), quaternion.identity, transform);
            gridline.transform.localScale = new Vector3(0.1f, nodelimits.height / 2, 0.1f);
        }
        for (int i = 1; i < nodelimits.height / 2; i++)
        {
            GameObject gridline = Instantiate(gridDot, new Vector3(0, i - nodelimits.height / 4, 0), quaternion.identity, transform);
